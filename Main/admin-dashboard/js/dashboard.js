...
onAuthStateChanged(auth, async (user) => {
    ...
    // Format email for mobile display
...
function formatEmailForMobile() {
    ...
    if (isMobile && email) {
        // Format: first 4-5 chars + ** + @domain
        const atIndex = email.indexOf('@');
        if (atIndex > 0) {
            const localPart = email.substring(0, atIndex);
            const domain = email.substring(atIndex); // includes @

            // Take first 4-5 characters (min 4, max 5 based on length)
            const prefixLength = Math.min(Math.max(4, Math.floor(localPart.length / 3)), 5);
            const prefix = localPart.substring(0, prefixLength);

            userEmailElement.textContent = `${prefix}**${domain}`;
        } else {
            // Fallback if no @ found (shouldn't happen with valid email)
            userEmailElement.textContent = email.substring(0, 8) + '**';
        }
    } else {
...
// Reformat email on window resize
let resizeTimeout;
